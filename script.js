(function () {
  ...
  function createGrid() {
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      // Update tooltip position to be near the cursor
    ...
    document.addEventListener('touchmove', (e) => {
      ...
      customCursor.style.left = `${touch.pageX - customCursor.offsetWidth / 2}px`;
      customCursor.style.top = `${touch.pageY - customCursor.offsetHeight / 2}px`;
    
      // Update tooltip position to be near the touch point
      customTooltip.style.left = `${touch.pageX + 15}px`;
      customTooltip.style.top = `${touch.pageY + 15}px`;
    });
    grid.innerHTML = "";
    cellElements = [];
    for (let i = 0; i < puzzle.length; i++) {
        ...
        cell.addEventListener('mouseenter', () => {
          // Show the tooltip when hovering over a grid cell
          customTooltip.textContent = 'Drag to select';  // Tooltip text
          customTooltip.style.display = 'block';  // Make tooltip visible
        });

        cell.addEventListener('touchstart', () => {
          // Show the tooltip when touching a grid cell
          customTooltip.textContent = 'Drag to select';  // Tooltip text
          customTooltip.style.display = 'block';  // Make tooltip visible
        });

        cell.addEventListener('mouseleave', () => {
          // Hide the tooltip when not hovering over a grid cell
          customTooltip.style.display = 'none';
        }
        );
        cell.addEventListener('touchend', () => {
          // Hide the tooltip when touch ends
          customTooltip.style.display = 'none';
        }
        );
...
function validateWord(word) {
  ...
  // Allow selection if word starts with any letter of already found words
  if (word.length > 0 && foundWords.size > 0) {
    const foundFirstLetters = Array.from(foundWords).map(w => w[0]);
    if (foundFirstLetters.includes(word[0])) {
      return true;
    }
  }
